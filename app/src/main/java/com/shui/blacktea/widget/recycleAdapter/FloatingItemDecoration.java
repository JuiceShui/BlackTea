...
/**
 * Description: 头部悬浮的效果，只支持LinearLayoutManager垂直滑动的情况
 * Created by -- on 2017/1/13.
 * <p>
 * Description: 代码规范
 * Modify by Juice_ on 2017/5/22 上午10:37
 */
...
public class FloatingItemDecoration extends RecyclerView.ItemDecoration {
    ...
    // 滚动列表的时候是否一直显示悬浮头部
    private boolean mShowFloatingHeaderOnScrolling = true;
    ...
    public FloatingItemDecoration(Context context, @ColorInt int color, @Dimension float dividerWidth, @Dimension float dividerHeight) {
        ...
        this.mDividerWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dividerWidth, context.getResources().getDisplayMetrics());
        this.mDividerHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, dividerHeight, context.getResources().getDisplayMetrics());
        init(context);
    }
...
//                Log.e(TAG, "onDrawOver: "+child.getTop()+"$"+firstVisiblePos );
    ...
    private void drawVertical(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (!mKeys.containsKey(params.getViewLayoutPosition())) {
                //画普通分割线
...
//                Log.e(TAG, "drawVertical: "+bottom );
                c.drawText(mKeys.get(params.getViewLayoutPosition()), x, y, mTextPaint);
            }
        }
    }

    public void setmShowFloatingHeaderOnScrolling(boolean mShowFloatingHeaderOnScrolling) {
        this.mShowFloatingHeaderOnScrolling = mShowFloatingHeaderOnScrolling;
    }

    public void setmKeys(Map<Integer, String> mKeys) {
        this.mKeys.clear();
